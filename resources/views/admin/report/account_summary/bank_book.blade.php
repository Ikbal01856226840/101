...
@section('title','Cash / Bank Book(s)')
@push('css')
<!-- model style -->
...
@component('components.report', [
    'title' => 'Cash / Bank Book(s)',
    'print_layout'=>'landscape',
    'print_header'=>'Cash / Bank Book(s)',
...
@slot('header_body')
<form id="cash_book_form" method="POST">
    @csrf
    {{ method_field('POST') }}
    <div class="row ">
        <div class="col-md-6">
    ...
    <table id="tableId" style=" border-collapse: collapse; " class="table table-striped customers tree table-scroll">
        <thead>
            <tr>
                ...
                <th class="th opening_checkbox">Opening Balance</th>
                <th class="th debit_checkbox">Debit Amount</th>
                <th class="th credit_checkbox">Credit Amount</th>
                <th class="th closing_checkbox">Closing Balance</th>
            </tr>
        </thead>
        ...
        <tfoot>
            <tr>
                ...
                <th style="font-size: 18px;" class="th total_opening opening_checkbox"></th>
                <th style="font-size: 18px;" class="th total_debit debit_checkbox"></th>
                <th style="font-size: 18px;" class="th total_credit credit_checkbox"></th>
                <th style="font-size: 18px;" class="th total_clasing closing_checkbox"></th>
            </tr>
        </tfoot>
    </table>
...
<script>
    ...
    $(document).ready(function() {
        // get bank book
        function get_bank_book_initial_show() {
            $(".modal").show();
            print_date();
            $.ajax({
                url: '{{ route("report-bank-book-data") }}',
                method: 'GET',
                data: {
                    to_date: $('.to_date').val(),
        ...
        get_bank_book_initial_show();

        $("#cash_book_form").submit(function(e) {
            print_date();
            $(".modal").show();
            total_opening = 0;
            total_debit = 0;
        ...
        // bank book function
        function get_bank_book(response) {
        ...
        arr.forEach(function(v) {
            ...
            if (v.under != 0) {
                if (chart_id != v.group_chart_id) {
                    ...
                    htmlFragments.push(`<tr id="${v.group_chart_id + '-' + v.under}" class='${under_id} table-row  ${group==v.under?'tree-node':''}'   data-id='${v.group_chart_id}' data-parent='${v.under}' > 
                                            <td style='width: 3%; border: 1px solid #ddd; font-size: 16px; color: #0B55C4'>
                                                <span class="group_chart">${accountGroupSummaryIdWise((v.group_chart_id + '-' + v.under),(h + a + v.group_chart_name))}</span>
                                                <span>
                ...
                if (v.ledger_head_id) {
                    let opening_blance = openning_blance_cal(v.nature_group,v.DrCr,v.opening_balance) + (v.nature_group == 1 || v.nature_group == 3 ? (v.op_total_debit || 0) - (v.op_total_credit || 0) : (v.nature_group == 2 || v.nature_group == 4 ? (v.op_total_credit || 0) - (v.op_total_debit || 0) : 0));
                    total_opening = (total_opening + opening_blance);
                    total_debit += (v.total_debit || 0);
                    total_credit += (v.total_credit || 0);
                    let clasing_blance = openning_blance_cal(v.nature_group,v.DrCr,v.opening_balance) + (v.nature_group == 1 || v.nature_group == 3 ? (((v.op_total_debit || 0) - (v.op_total_credit || 0) + (v.total_debit || 0)) - (v.total_credit || 0)) : (v.nature_group == 2 || v.nature_group == 4 ? ((v.op_total_credit || 0) - (v.op_total_debit || 0) + (v.total_credit || 0)) - (v.total_debit || 0) : 0));
                    ...
                    htmlFragments.push(`<tr id="${v.ledger_head_id}" class="${under_id} ledger_id table-row tree-node " data-id_data-parent="${v.under}" data-parent="${v.group_chart_id}"  >
                                                <td style='width: 3%;  border: 1px solid #ddd; font-size: 16px; color: #0B55C4'><p style="margin-left:${(h + a + a + a).length}px" class="text-wrap mb-0 pb-0 ">${accountLedgerVoucherIdWise(v.ledger_head_id,v.ledger_name)}</p></td>
                                                ...
                                                <td class='td'> ${(clasing_blance).toFixed(amount_decimals).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</td>
                                        </tr>`);
                }
            }
            if ('children' in v) {
