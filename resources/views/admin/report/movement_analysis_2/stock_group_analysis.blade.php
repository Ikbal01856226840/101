...
@component('components.report_setting_modal', [
    ...
    'class' =>'modal fade',
    'size'=>'modal-xl',
    'page_title'=>'StockGroupAnalysis',
    'page_unique_id'=>7,
    'godown'=>'yes',
    'stockGroup'=>'yes',
    'title'=>'StockGroupAnalysis',
    'daynamic_function'=>'get_stock_group_analysis_initial_show',
])
@endcomponent
<!-- add component-->
@component('components.report', [
    ...
    'user_privilege_title'=>'StockGroupAnalysis',
    'print_date'=>1,
                ...
                <div class="row mt-2">
                    ...
                    <div class="row ps-4">
                        ...
                        <div class="col-md-2 col-sm-4 col-4">
                            @if (company()->sales_return==2)
                              <input class="form-check-input purchase_return_in in_qty" type="checkbox" name="in_qty[]" {{ isset($purchase_return_in)?($purchase_return_in==29 ? 'checked' : ''):'' }} value="29" {{$purchase_return_in ?? "checked"}}>
                            @elseif (company()->sales_return==1)
                              <input class="form-check-input purchase_return_in out_qty" type="checkbox" name="out_qty[]" {{ isset($purchase_return_in)?($purchase_return_in==29 ? 'checked' : ''):'' }} value="29" {{$purchase_return_in ?? "checked"}}>
                            @endif
                    ...
                    <div class="row ps-4">
                        ...
                        <div class="col-md-2 col-sm-4 col-4">
                            ...
                            @elseif (company()->sales_return==1)
                                <input class="form-check-input sales_return_out in_qty" type="checkbox" {{ isset($sales_return_out)?($sales_return_out==25 ? 'checked' : ''):'' }} name="in_qty[]" value="25" {{$sales_return_out ?? "checked"}}>
                             @endif
    ...
    <table id="tableId" style=" border-collapse: collapse; " class="table table-striped customers table-scroll">
        <thead>
            <thead>
                <tr>
                    ...
                    <th colspan="3"  style=" width: 5%;" class="th inwards_text text-center in_wards"  id="inwards_text">Inward : Purchase, GRN, Sales Return, Journal , Stock Journal	</th>
                    <th colspan="3" style=" width: 5%;" class="th outwards_text text-center out_wards"  id="outwards_text">Outward :Sales , GTN, Purchase Return, Journal , Stock Journal</th>
                </tr>
                <tr>
                    <th  style="width: 2%; overflow: hidden;" class="th text-end ">Quantity</th>
                    <th  style="width: 2%; overflow: hidden;" class="th text-end in_wards_header_rate">Rate</th>
                    <th  style="width: 3%; overflow: hidden;" class="th text-end ">Value</th>
                    <th  style="width: 2%; overflow: hidden;" class="th text-end " >Quantity</th>
                    <th  style="width: 2%; overflow: hidden;" class="th text-end out_wards_header_rate">Rate</th>
                    <th  style="width: 3%; overflow: hidden;" class="th text-end ">Value</th>
                </tr>
        </thead>
        ...
        <tfoot>
            <tr>
                ...
                <th style="width: 3%; font-size: 18px;"  class="th total_inwards_rate text-end in_wards_header_rate in_wards"></th>
                <th style="width: 2%; font-size: 18px;"  class="th total_outwards_value text-end in_wards"></th>
                <th style="width: 3%; font-size: 18px;"  class="th total_outwards_qty text-end out_wards"></th>
                <th style="width: 2%; font-size: 18px;"  class="th total_outwards_rate text-end out_wards_header_rate out_wards"></th>
...
$(document).ready(function () {
    ...
    if("{{$stock_group_id??0}}"!=0){
        ...
        local_store_stock_group_analysis_get()
    }
    get_stock_group_analysis_initial_show();
    
    $("#stock_group_analysis").submit(function(e) {
        local_store_stock_group_analysis_set_data();
        print_date();
        checkbox_check('','');
...
function get_stock_group_analysis(response){
    ...
    var tree=getTreeView(response.data,children_sum);
       $('.item_body').html(tree);
        ...
        $('.total_outwards_value').text(total_inwards_value.formatBangladeshCurrencyType("amount"));
    ...
    arr.forEach(function (v) {
        ...
        if (chart_id != v.stock_group_id) {
            ...
            if (((matchingChild.stock_qty_in|| 0) == 0) && ((matchingChild.stock_qty_out || 0) == 0) ) {} else {
                ...
                html.push(`<td style='width: 3%; color: #0B55C4; border: 1px solid #ddd;font-weight: bold;' ><p style="margin-left:${(h+a+a).length-12}px; font-family: Arial, sans-serif;" class="text-wrap mb-0 pb-0 ">${v.stock_group_name}</p></td>`);
                ...
                if (matchingChild) {
                    ...
                    if(in_ward_column_rate){
                        html.push(`<td style='width: 3%;font-weight: bold;' class="td  text-end in_wards">
                    ...
                    if(out_ward_column_rate){
                        html.push(`<td style='width: 3%;font-weight: bold;' class="td  text-end out_wards">
        ...
        if (((v.stock_qty_total_in|| 0) == 0) && ((v.stock_qty_total_out || 0) == 0) ) {} else {
            ...
            if(in_ward_column_rate){
                html.push(`<td style='width: 3%; 'class='td inwards  text-end '>
                    ${(((v.stock_total_value_in||0)/(v.stock_qty_total_in||0))||0).formatBangladeshCurrencyType("rate")}
                    </td>`);
            }            
            ...
            if(out_ward_column_rate){
                html.push(`<td style='width: 3%; 'class='td outwards text-end '>
                    ${(((v.stock_total_value_out||0)/(v.stock_qty_total_out||0))||0).formatBangladeshCurrencyType("rate")}
                    </td>`);
            }
...
function local_store_stock_group_analysis_get() {
    ...
    function local_store_stock_group_analysis_set_data() {
