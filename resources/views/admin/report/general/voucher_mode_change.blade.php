...
@section('title','Voucher Mode Change')
@push('css')
<style>
    input[type=radio] {
        width: 20px;
...
@component('components.modal', [
    'id' => 'VoucherModeChange',
    'class' => 'modal fade',
    'form_id' => 'FormVoucherModeChange',
    'method'=> 'POST',
    'title'=>'Voucher Mode Change'
])
@slot('body')
<input type="hidden" name="transaction_id" id="transaction_id">
<div class="row" id="voucher_mode_change_modal_body"></div>
@endslot
@slot('footer')
<div id="voucher_mode_change_modal_footer">
    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
    <button type="button" class="btn btn-primary change_voucher_mode_save">Save</button>
</div>
@endslot
@endcomponent
<!-- add component-->
@component('components.report', [
    ...
    'print_header'=>'Voucher Mode Change',
    'user_privilege_title'=>'VoucherModeChange',
    'print_date'=>1,
]);
...
@slot('header_body')
<form id="VoucherModeChangeForm" method="POST">
    @csrf
    {{ method_field('POST') }}
    <div class="row">
        <div class="col-md-2">
...
<script>
    ...
    $(document).ready(function() {
        ...
        function get_daybook_initial_show() {
            ...
            $(".modal").show();
            $.ajax({
                url: "{{ route('report.voucher.mode.change.data') }}",
                type: 'GET',
                dataType: 'json',
                data: {
                    to_date: $('.to_date').val(),
        ...
        // day book  show
        $("#VoucherModeChangeForm").submit(function(e) {
            e.preventDefault();
            print_date();
            $(".modal").show();
                ...
                htmlFragments.push(`
                    <tr class="left left-data editIcon table-row">
                        ...
                        <td style="width: 2%;" class="td font text-wrap">
                            ${redirectVoucherIdWise(v.voucher_type_id,v.tran_id,v.voucher_name)}                        </td>
                        ...
                        <td style="width: 3%;" class="td font">
                            <button type="button" class="btn btn-sm btn-success voucher_mode_change" value="${v.tran_id}" tran_id="${v.tran_id}" voucher_id="${v.voucher_id}" voucher_type_id="${v.voucher_type_id}">Convert</button>
                        </td>
                `);
    ...
    $(document).on('click','.voucher_mode_change', function() {
        let voucher_list=vouchers?.filter(v=>v.voucher_type_id==$(this).attr('voucher_type_id'))?.
                    map(x=>`<div class="m-2"><input type="radio" name="voucher_id" class=" new_voucher_id" value="${x.voucher_id}"><span class="ms-2">${x.voucher_name}</span></div>`
                    ).join('');
        $("#transaction_id").val($(this).attr('tran_id'));
        $('#voucher_mode_change_modal_body').html(voucher_list);
        $('#VoucherModeChange').modal("show");

    })
    $('.change_voucher_mode_save').click(function(e) {
        let voucher_id=$('.new_voucher_id:checked').val();
        let transaction_id=$("#transaction_id").val();
        e.preventDefault();
        // const fd = new FormData(document.getElementById('VoucherNumberModifyFormUpdate'));;
        const fd = new FormData();
        fd.append('_token', '{{ csrf_token() }}');
        fd.append('voucher_id', voucher_id);
        fd.append('tran_id', transaction_id);
        console.log(fd);
        $.ajax({
            url: "{{  route('report.voucher.mode.change.store') }}",
            method: 'POST',
            data: fd,
            cache: false,
            contentType: false,
