...
@section('title','Stock Item Monthly Summary Store')
@push('css')
 <style>
...
'id' =>'exampleModal',
'class' =>'modal fade',
'page_title'=>'Stock Item Monthly Summary Store',
'size'=>'modal-xl',
'page_unique_id'=>18,
'godown'=>'yes',
'stock_item'=>'yes',
'title'=>'Stock Item Monthly Summary Store',
'daynamic_function'=>'get_stock_item_monthly_summary_initial_show'
])
@endcomponent
<!-- add component-->
@component('components.report', [
    ...
    'print_header'=>'Stock Item Monthly Summary Store',
    'user_privilege_title'=>'StockItemMonthlyStore',
    'report_setting_model'=>'report_setting_model Store',
...
<div class="dt-responsive table-responsive cell-border sd tableFixHead_item_register">
    <table id="tableId" style=" border-collapse: collapse; " class="table table-striped customers table-scroll">
        <thead>
            <tr>
                <th style="width: 1%;" class="align-middle">SL.</th>
                <th style="width: 3%;  border: 1px solid #ddd;" class="align-middle">Date</th>
                <th class="th inwards text-end"  style=" width:5%">Inward Quantity</th>
                <th class="th outwards text-end" style=" width:5%;">Outward Quantity</th>
                <th class="th clasing text-end"  style=" width:5%;">Closing Quantity</th>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // stock item get id check
        if("{{$stock_item_id??0}}"!=0){
            local_store_stock_group_monthly_summary_set_data();
        }else{
            local_store_stock_group_monthly_summary_get();
        }
    ...
    function get_stock_item_monthly_summary_val(response) {
        ...
        response.current_stock.forEach((v, key) => {
            ...
            <td class="td" style="width: 3%;color: #0B55C4;" class="text-wrap date">
                ${stockItemRegisterStoreMonthlyIdWise(v.transaction_date,new Date(v.transaction_date).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }))}
            </td>
    ...
    function get_stock_item_monthly_summary_initial_show(){
        ...
        $(".modal").show();
        $.ajax({
            url: '{{ route("stock-item-monthly-summary-store-data") }}',
                method: 'GET',
                data: {
                    to_date:$('.to_date').val(),
    ...
    function local_store_stock_group_monthly_summary_get() {
    ...
    function local_store_stock_group_monthly_summary_set_data() {
    ...
    //     url = "{{route('stock-item-register-store-monthly-id-wise', ['date' =>':date', 'stock_item_id'=>':stock_item_id','godown_id'=>':godown_id'])}}";
