...
@section('title','Stock Item Register Store')
@push('css')
 <style>
...
'id' =>'exampleModal',
'class' =>'modal fade',
'page_title'=>'Stock Item Register Store',
'size'=>'modal-xl',
'page_unique_id'=>19,
'godown'=>'yes',
'stock_item'=>'yes',
'title'=>'Stock Item Register Store',
...
@component('components.report', [
    ...
    'print_header'=>'Stock Item Register Store',
 ...
 <form  id="add_item_register_form"  method="POST">
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <label></label>
            <div>
                <input class="form-check-input inwords_qty_checkbox" type="checkbox"  name="inwords_qty"  checked>
                <label class="form-check-label fs-6" for="flexRadioDefault1" >
                    Inwards Quantity
                </label>
                <input class="form-check-input outwords_qty_checkbox" type="checkbox"  name="outwords_qty"   checked>
                <label class="form-check-label fs-6" for="flexRadioDefault1">
                    Outwards  Quantity
                </label>
...
<div class="dt-responsive table-responsive cell-border sd tableFixHead_item_register">
    <table id="tableId" style=" border-collapse: collapse; " class="table table-striped customers table-scroll">
        <thead>
            <tr>
                <th style="width: 1%; text-align:center;" class="th">SL.</th>
                <th style="width: 3%" class="th" >Date</th>
                <th style="width: 3%;" class="th">Voucher No</th>
                <th style="width: 2%;" class="th">Voucher Type</th>
                <th style="width: 5%;table-layout: fixed;" class="th narration">Narration</th>
                <th style="width: 5%;table-layout: fixed;" class="th">Particulars</th>
                <th style=" width: 5%"class="th inwards  text-end">Inward Balance</th>
                <th style=" width: 5%;"class="th outwards text-end">Outward Balance</th>
                <th style=" width: 5%; "class="th closing text-end">Closing Balance</th>
            </tr>
        ...
        <tfoot>
            <tr>
                ...
                <th  style="width: 2%;font-size: 18px;" class="th inwards total_inwards_qty text-end"></th>
                <th  style="width: 2%;font-size: 18px;" class="th outwards total_outwards_qty text-end"></th>
                <th  style="width: 3%;font-size: 18px;" class="th closing total_clasing_qty text-end"></th>
...
let  total_inwards_qty=0; total_outwards_qty=0;total_clasing_qty=0;
...
function get_item_register_val(response) {
        ...
        row.innerHTML=`<td style="width: 1%; border: 1px solid #ddd;"></td>
                        ...
                        ${$(".inwords_qty_checkbox").is(':checked') ? `<td style="width: 2%;" class="td"></td>` : ''}
                        ${$(".outwords_qty_checkbox").is(':checked') ? `<td style="width: 2%;" class="td"></td>` : ''}
        ...
        $.each(response.current_stock, function (key, v) {
                ...
                row.innerHTML = `
                    ...
                    <td style="width: 2%;" class="td">${redirectVoucherIdWise(v.voucher_type_id, v.tran_id,v.ledger_name)}</td>
                    ...
                    ${$(".inwords_qty_checkbox").is(':checked') ? `<td style="width: 3%;" class="td text-end" >${(inwards_qty_in || 0).formatBangladeshCurrencyType("quantity", response?.unit_of_measure?.symbol)}</td>` : ""}
                    ${$(".outwords_qty_checkbox").is(':checked') ? `<td style="width: 3%;" class="td text-end">${(outwards_qty_o||0).formatBangladeshCurrencyType("quantity", response?.unit_of_measure?.symbol)}</td>` : ""}
    ...
    function get_stock_item_register_initial_show(){
        ...
        $(".modal").show();
        $.ajax({
            url: '{{ route("stock-item-register-store-data") }}',
                method: 'GET',
                data: {
                    to_date:$('.to_date').val(),
    ...
    function checkbox_check(){
        ...
        //checking condition
        $(".inwards").css("display", $(".inwords_qty_checkbox" ).is(':checked')==true?'':'none');
        $(".outwards").css("display", $(".outwords_qty_checkbox" ).is(':checked')==true?'':'none');
    ...
    function local_store_stock_item_register_get() {
        ...
        getStorage("inwords_qty_checkbox", '.inwords_qty_checkbox', 'checkbox');
        getStorage("outwords_qty_checkbox", '.outwords_qty_checkbox', 'checkbox');
    ...
    function local_store_stock_item_register_set_data() {
        ...
        setStorage("inwords_qty_checkbox", $(".inwords_qty_checkbox").is(':checked'));
        setStorage("outwords_qty_checkbox",$(".outwords_qty_checkbox").is(':checked'));
