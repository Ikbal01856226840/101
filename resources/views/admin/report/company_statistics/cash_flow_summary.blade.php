...
<!-- Main body component -->
@slot('main_body')
<div class="dt-responsive table-responsive cell-border sd cash_flow_summarytableFixHead">
        ...
        if(v.under!=0){
                    ...
                    let matchingChild = children_sum.find(c =>v.group_chart_id == c.group_chart_id);
                     if (((matchingChild.group_debit|| 0) == 0) && ((matchingChild.group_credit || 0) == 0)) {} else {
                        html.push(`<tr id="${v.group_chart_id}" class='${under_id} table-row  ${group==v.under?'tree-node':''} left left-data'   data-id='${v.group_chart_id}' data-parent='${v.under}'>
                                    ...
                                    <td style='width: 3%;  border: 1px solid #ddd; font-size: 16px; color: #0B55C4' class="td-bold"><span class="group_chart">${h+a+v.group_chart_name}</span>
                                                <span>
                            ...
                            if (matchingChild) {
                                html.push(`<td style='width: 3%;' class="td text-end td-bold">${(matchingChild.group_debit || 0).formatBangladeshCurrencyType("accounts")}</td>
                                        <td style='width: 3%;' class="td text-end td-bold">${((((matchingChild.group_credit||0))||0).formatBangladeshCurrencyType("accounts"))}</td>
                                    </tr>`);
                            }
                     }
            ...
            if (((v.op_total_debit|| 0) == 0) && ((v.op_total_credit || 0) == 0)) {} else {
                ...
                html.push(`<tr id="${v.ledger_head_id}" class="${under_id} ledger_id table-row tree-node " data-id_data-parent="${v.under}" data-parent="${v.group_chart_id}">
                               ...
                               <td style="width: 5%; color: #0B55C4" class="td">${h+a+a+v.ledger_name}</td>
                               <td style='width: 3%;'class='td opening text-end'>${(v.op_total_debit || 0).formatBangladeshCurrencyType("accounts")}</td>
                              <td style='width: 3%; 'class='td inwards text-end'>${(v.op_total_credit||0).formatBangladeshCurrencyType("accounts")}</td>
                          </tr>`);
            }
        }
            if ('children' in v) {
...
$(document).ready(function () {
    ...
    $('.cash_flow_summarytableFixHead').css('height',`${display_height-120}px`);
});
...
function local_store_cash_flow_summary_get() {
        getStorage("end_date", '.to_date');
        getStorage("start_date", '.from_date');
    }

function local_store_cash_flow_summary_set_data() {
