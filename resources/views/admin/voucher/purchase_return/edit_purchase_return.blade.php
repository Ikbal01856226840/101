...
<form id="edit_purchase_return_id" method="POST">
    ...
    <div align="center">
        @if (user_privileges_check('Voucher',$voucher->voucher_id,'alter_role'))
            ...
            <button type="submit" class="btn btn-info edit_purchase_return_btn" style="width: 120px; border-radius: 15px;">
            ...
            $.each(response.data, function(i,val) {
                ...
                <td  class="m-0 p-0">
                      <input class="form-control stock text-right"   data-field-name="stock" type="number"  step="any"
                       value="${(parseFloat((val.stock_in_sum)-(val.stock_out_sum))).toFixed(amount_decimals)}"
                       id="stock_${i}"  for="${i}" readonly />
                </td>
                <td  class="m-0 p-0">
                    <input class="form-control qty text-right" name="qty[]"  data-field-name="qty"
                    type="number" step="any"
                     id="qty_${i}" value="${Math.abs(val.qty)}" for="${i}"  />
                </td>
                <td  class="m-0 p-0">
                    <input class="form-control rate text-right"  name="rate[]" data-field-name="rate"
                    type="number" step="any"
                    data-type="rate" id="rate_${i}" value="${Math.abs(val.rate).toFixed(amount_decimals)}"
                ...
                <td  class="m-0 p-0">
                    ...
                    name="amount[]" id="amount_${i}"  value="${Math.abs(val.total).toFixed(amount_decimals)}"
                    ${amount_typeabe==1?'readonly':''}  for="${i}"/>
                </td>
      ...
      for(var row=1; row<6;row++) {
          rowCount++;
              $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${rowCount}">
                  ...
                  <td  class="m-0 p-0">
                      <input class="form-control amount  text-right" type="number"  name="amount[]"  step="any"
                      id="amount_${rowCount}" ${amount_typeabe==1?'readonly':''}  for="${rowCount}"/>
                  </td>
                  <td  class="m-0 p-0 ${remark_is==0?'d-none':''}">
