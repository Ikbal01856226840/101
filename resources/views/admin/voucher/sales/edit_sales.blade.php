...
<form id="edit_sales_id" method="POST">
            ...
            <div class="col-sm-4">
                <div class="row">
                    ...
                    <div class="col-sm-8 m-0 p-0" style="margin-left: 5px;!important;">
                        ...
                        <input type="text" name="invoice_no" class="form-control m-1" value="{{$data->invoice_no}}" style="border-radius: 15px;" style="color: green;" required readonly/>
                        ...
                        @if (company()->gate_pass_ref!=0)
                            ...
                            <input type="text" name="gprf" class="form-control m-1" value="{{$data->gprf??''}}" style="border-radius: 15px;" autofocus />
                        @endif
    ...
    <div class="row  {{ $data->sale_type==1?'':'d-none' }} cash_ledger mt-0 pt-0" >
        <div class="col-sm-3 mt-0 pt-0"  >
            <label class="salse_cash_ledger_amount" style="font-weight: bold; font-size: 18px !important; margin: 0px;">{{$cash_credit_sum_value??'0.000' }}</label>
            <div class="form-group" >
                ...
                value="{{$cash_ledger_name->ledger_name??''}}"
                disabled="{{ $data->sale_type==1}}"
               />
               <input  type="hidden"  name="salse_cash_ledger_id" value="{{$data->salse_cash_ledger_id??0 }}" class="form-control salse_cash_ledger_id"  {{ $data->sale_type==1?'':'disabled' }}/>
            ...
            <table class="table customers" style="border: none !important; margin-top: 5px;">
                <thead>
                    <tr>
                        ...
                        <th class="th {{$voucher->commission_is==1?'col-2':'col-2'}}">Amount</th>
                        @if($voucher->commission_is==1)
                ...
                <tfoot>
                    ...
                    <tr>
                        ...
                        <td>
                            <input
                                type="number"
                                name="get_without_commission"
                                style="font-weight: bold;"
                                class="form-control text-right get_commission_without"
                                step="any"
                                readonly
                            />
                        </td>
                    ...
                    <tr>
                        ...
                        <td colspan="2">
                            <input
                                type="number"
                                step="any"
                                name="product_wise_get_commission_without"
                                style="font-weight: bold;"
                                class="product_wise_get_commission_without form-control text-right"
                                readonly
                            /></td>
                        @endif
                        <td class="col-1 {{$voucher->remark_is==0?'d-none':'' }}"></td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input
                                ...
                                style="font-weight: bold;"
                                name="product_wise_get_commission_without"
                                class="form-control text-right product_wise_get_commission_without"
                                readonly
                            />
                        </td>
                        <td class="col-1 {{$voucher->remark_is==0?'d-none':'' }}"></td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input
                                ...
                                name="nettotal"
                                style="font-weight: bold;"
                                class="form-control text-right nettotal"
                                readonly
                            />
                        </td>
                        <td class="col-1 {{$voucher->remark_is==0?'d-none':'' }}"></td>
...
function selected_val(){
    ...
    $('.sale_type').val('{{$data->sale_type??0}}').trigger('change');
    if(commission_is!=1){
    $('.debit_ledger_id').val('{{$credit->ledger_head_id??0}}');
    }
}
...
party_ledger_autocomplete("{{$voucher->voucher_id}}",'salse_cash_ledger_amount','salse_cash_ledger_name','salse_cash_ledger_id','salse_cash_value_affected','{{route("searching-ledger-debit") }}','{{route("balance-debit-credit") }}');
        ...
        success: function (response) {
            $.each(response.data, function(i,val) {
                $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${i}">
                    ...
                    <td  class="m-0 p-0">
                        <input class="form-control actual_qty text-right" name="actual_qty[]"  type="hidden" step="any"  id="actual_qty_${i}" value="${val.qty}" for="${i}"  />
                        <input
                    ...
                    `<td  class="m-0 p-0">
                        ...
                        <select   name="debit_ledger_id[]"  class="form-control debit_ledger_id${val.credit_id} ledger_debit" >
                        <option value="0">--Select--</option>
                        ${ledger_tree}
                        </select>
                    </td>
                    <td  class="m-0 p-0">
                        ...
                        <input class="commission_id" type="hidden" name="commission_id[]" value="${val.commission_id??0}">
                    ...
                    <td  class="m-0 p-0">
                        <div style="display: flex;flex-direction: row;">
                                ...
                                <input type="number" step="any" name="product_wise_commission_amount[]" id="product_wise_commission_amount_${i}" value="${val.commission_commission?val.commission_commission:0}" class="product_wise_commission_amount form-control text-right mx-0 px-0" style="min-width:60px;" >
                        </div>
                    </td>
                    <td class="m-0 p-0"><input type="number" step="any" name="product_wise_get_commission[]"  id="product_wise_get_commission_${i}" value="${val.commission_debit?val.commission_debit:(val.commission_credit?val.commission_credit:0)}" class="product_wise_get_commission form-control text-right mx-0 px-0" readonly></td>
                    `:`""`}
        ...
        for(var row=1; row<6;row++) {
            ...
            rowCount++;
                $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${rowCount}">
                    ...
                    <td  class="m-0 p-0">
                        <input class="form-control amount  text-right" type="number"   step="any"
                        name="amount[]" id="amount_${rowCount}" ${amount_typeabe==1?'readonly':''}  for="${rowCount}"/>
                    </td>
                    ${(commission_is==1)?
                    `<td  class="m-0 p-0">
                        <select   name="debit_ledger_id[]"  class="form-control  debit_ledger_id_commission" id="ledger_debit_id_${rowCount}">
                        <option value="0">--Select--</option>
                       ${ledger_tree}
                        </select>
                    </td>
    ...
    function commission_per_calcucation(){
        ...
        $('#commission_append tr').each(function(i){
            ...
            if(commision_cal==1 || commision_cal==2){
                ...
                console.log(commissionValue,total_amount);
    ...
    $.each(JSON.parse(debit_credit_value.replace(/&quot;/g,'"')), function(i,val) {
        if(val.commission_type&&val.comm_level==1){
            $('#commission_append').append(`<tr id="${i}">
                ...
                <td class="m-0 p-0" >
                    <input type="number" name="commission_amount[]"  step="any"
                    class="form-control commission_amount" value="${val.commission.toFixed(amount_decimals)}"
                     id="commission_amount_${i}" />
                </td>
       ...
       commission_calcucation();
   var ComRowCount=$('#commission_append tr:last').attr('id')||1;
   $('#add_commission').click(function() {
      ComRowCount++;
        $('#commission_append').append(`<tr id="${ComRowCount}">
                ...
                <td class="text-right ">Commission: </td>
                <td  colspan="${godown_motive==3 || godown_motive==4?2:3}" class="m-0 p-0" > <select name="commission_ledger_id[]" class="form-control select2 commission_select2" id="select_${ComRowCount}" required>
                <option value="0">--Select--</option>
                     ${ledger_commission_tree}
                </select></td>
                <td class="m-0 p-0">
                ...
                <td class="m-0 p-0" >
                    <input type="number" name="commission_amount[]" step="any"
                    class="form-control commission_amount" id="commission_amount_${ComRowCount}" />
                </td>
...
$(document).on('click', '.CancelIcon', function(e) {
        ...
        swal(swal_data()).then((result) => {
            if (result.value) {
                event.preventDefault();
                $.ajax({
                    url: "{{ url('voucher-sales-cancel') }}" + '/' + id ,
                    type : "POST",
...
function select_val(){
    ...
    $('.unit_or_branch').val('{{$data->unit_or_branch}}');
    $('.dis_cen_id').val('{{$data->dis_cen_id}}');
    $('.sale_type').val('{{$data->sale_type??0}}');
    var debit_credit_value='{{$debit_credit_data}}';
    var commission_is="{{$voucher->commission_is}}";
    if(commission_is!=1){
        $('.debit_ledger_id').val('{{$credit->ledger_head_id??0}}');
