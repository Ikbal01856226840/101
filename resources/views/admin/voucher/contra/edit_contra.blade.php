...
<form id="edit_contra_id" method="POST">
    ...
    <div align="center">
        @if (user_privileges_check('Voucher',$voucher->voucher_id,'alter_role'))
            ...
            <button type="submit" class="btn btn-info add_contra_btn" style="width: 120px; border-radius: 15px;">
...
@push('js')
  ...
  <script>
   $(document).ready(function(){
        ...
        $('#orders').on('keyup','.display_credit, .display_debit ',function(){
            if($(this).attr('class').search('display_credit')>=0){
                $(this).closest('tr').find('.credit').val($(this).val());
            }else if($(this).attr('class').search('display_debit')>=0){
                $(this).closest('tr').find('.debit').val($(this).val());
            }
            button_debit_or_credit_total();
...
$(document).on('click', '.CancelIcon', function(e) {
        ...
        swal(swal_data()).then((result) => {
            if (result.value) {
                event.preventDefault();
                $.ajax({
                    url: "{{ url('voucher-contra-cancel') }}" + '/' + id ,
                    type : "POST",
