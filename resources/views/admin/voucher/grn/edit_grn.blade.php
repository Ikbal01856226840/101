        ...
        <div class="table-responsive">
            <table class="table customers" style="border: none !important; margin-top: 5px;">
                <thead>
                    <tr>
                        <th class="col-0.5">#</th>
                        <th class="th {{$voucher->remark_is==1?'col-4':'col-5'}}">Product Name</th>
    ...
    <div align="center">
        @if (user_privileges_check('Voucher',$voucher->voucher_id,'alter_role'))
            @if (voucher_modify_authorization($data->tran_id))
                <button type="submit" class="btn btn-info edit_grn_btn cansale_btn" style="width: 120px; border-radius: 15px;">
...
function selected_val(){
    ...
    $('.debit_ledger_id').val('{{$debit->ledger_head_id??0}}');
}
// credit ledger searching
$(document).ready(function () {
     // form reset
     resetForm("edit_grn_id");
     selected_val();
  ...
  // get stock item
    $.ajax({
            ...
            $.each(response, function(i,val) {
                ...
                <td  class="m-0 p-0">
                    <input class="form-control qty text-right" name="qty[]"  data-field-name="qty" step="any" type="number" class="qty" id="qty_${i}" value="${Math.abs(val.qty)}" for="${i}"  />
                </td>
                <td  class="m-0 p-0">
        ...
        for(var row=1; row<6;row++) {
            ...
            rowCount++;
                $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${rowCount}">
                    ...
                    <td  class="m-0 p-0">
                        <input class="form-control rate text-right"
                            name="rate[]" data-field-name="rate" type="number" step="any"
                            data-type="rate" id="rate_${rowCount}"  for="${rowCount}"
                    ...
                    <td  class="m-0 p-0">
                        <input class="form-control amount  text-right" type="number"  step="any"
                            name="amount[]" id="amount_${rowCount}" ${amount_typeabe==1?'readonly':''}
                            for="${rowCount}"
                        />
                    </td>
                    <td  class="m-0 p-0 ${remark_is==0?'d-none':''}">
                ...
                $('#commission_append').append(`<tr id="${i}">
                    ...
                    <td class="m-0 p-0" >
                        <input
                            ...
                            name="commission_amount[]" class="form-control commission_amount"
