...
@push('js')
    <script>
        ...
        const debit_credit_amont= 1;
        const remark_is="{{$voucher->remark_is ?? ''}}";
        const dc_amnt="{{$voucher->dc_amnt ?? ''}}";
        const stock_item_price_typeabe="{{$voucher->stock_item_price_typeabe ?? ''}}";
        const total_qty_is="{{$voucher->total_qty_is ?? ''}}";
        const total_price_is="{{$voucher->total_price_is ?? ''}}";
        const amount_typeabe="{{$voucher->amount_typeabe ?? ''}}";
        const godown_motive="{{$voucher->godown_motive ?? ''}}";
        const t_m_id="{{$data->tran_id}}";
    </script>
    ...
    $(document).ready(function(){
                ...
                $('#orders').append(`<tr style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${i}">
                            ...
                            <td  class="m-0 p-0">
                                <button  type="button" name="remove" id="${i}"
                                    class="btn btn-danger btn_remove cicle m-0  py-0">-</button>
                            </td>
                            ...
                            <input class="form-control  nature_group  m-0 p-0" type="hidden" name="nature_group[]" data-type="nature_group" id="nature_group_${i}">
                            ${(val.debit_credit_id!=debit_credit_check_in) ? `
                                ...
                                <td  class="m-0 p-0">
                                    <input
                                        ...
                                        autocomplete="off"
                                        for="${i}"
                                        value="${val.ledger_name?val.ledger_name:''}"
                                    />
                            ...
                            <td  class="m-0 p-0">
                                <input
                                    ...
                                    autocomplete="off"
                                    for="${i}"
                                    value="${val.in_product_name?val.in_product_name:(val.out_product_name?val.out_product_name:'')}"
                                />
                            ...
                            <td  class="m-0 p-0 ${godown_motive==3?'d-none':''} ${godown_motive==4?'d-none':''}">
                                <input
                                    ...
                                    type="text"
                                    id="godown_name_${i}"
                                    for="${i}" ${godown_motive==2?'readonly':''}
                                    autocomplete="off"
                                    value="${val.in_godown_name?val.in_godown_name:(val.out_godowns_name?val.out_godowns_name:'')}"
                                />
                            ...
                            <td  class="m-0 p-0">
                                <input class="form-control qty text-right " name="qty[]"  data-field-name="qty" type="number" value="${val.in_qty?val.in_qty:val.out_qty}" step="any" class="qty" id="qty_${i}"  for="${i}"  />
                            </td>
                            <td  class="m-0 p-0">
                                <input class="form-control rate text-right "   name="rate[]" data-field-name="rate" type="number" value="${val.in_rate?val.in_rate:(val.out_rate?val.out_rate:(val.rate||0))}"  step="any" data-type="rate" id="rate_${i}"  for="${i}" ${stock_item_price_typeabe==0?'readonly':''} />
                            </td>
                            ...
                            <td  class="m-0 p-0">
                                <input
                                    ...
                                    step="any"
                                    name="display_amount[]"
                                    value="${MakeCurrency(val?.in_total||val?.out_total||'',true,amount_decimals)}"
                                    amount="${val?.in_total||val?.out_total||''}"
                                    step="any"
                                ...
                                <input
                                    ...
                                    step="any"
                                    name="amount[]"
                                    value="${MakeCurrency(val.in_total?val.in_total:val.out_total,true,amount_decimals)}"
                                    amount="${val.in_total?val.in_total:val.out_total}"
                                    step="any"
                                    id="amount_${i}"
                                    ${amount_typeabe==1?'readonly':''}
                                    for="${i}"
                        ...
                        ${remark_is==1 && `<td  class="m-0 p-0 "><input class="form-control remark" value="${val.remark==null?"":val.remark}"  name="remark[]" type="text" data-type=" id="remark_${i}"  autocomplete="off" for="${i}"/></td>`}
                    </tr>`);
                    adjustFontSize($(`#godown_name_${i}`)[0],val.godown_name);
                    adjustFontSize($(`#product_name_${i}`)[0],val.product_name);
                    adjustFontSize($(`#ledger_name_${i}`)[0],val.ledger_name);
        ...
        function addrow (rowCount)
        {
                ...
                for(var row=1; row<6;row++) {
                        ...
                        $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${rowCount}">
                            ...
                            <td  class="m-0 p-0">
                                <input
                                    class="form-control stock_out text-right"
                                    data-field-name="stock_out"
                                    type="number"
                                    class="stock"
                                    id="stock_${rowCount}"
                                    for="${rowCount}"
                    ...
                    if(check_blance==-1){
                            if(debit_credit_amont==0){
                                if(parseFloat($(this).closest('tr').find('.blance').val())<parseFloat(debit)){
                                    ...
                                    StoreCreditValue.call($(this).closest('tr'),debit)
                                }
                          }else{
                            // $(this).closest('tr').find('.credit').val(debit);
                            // $(this).closest('tr').find('.display_credit').val(debit);
                    ...
                    if(check_blance==-1){
                            if(debit_credit_amont==0){
                                if(parseFloat($(this).closest('tr').find('.blance').val())<parseFloat(credit)){
                                    ...
                                    StoreDebitValue.call($(this).closest('tr'),credit)
                                }
                          }else{
                            //  $(this).closest('tr').find('.debit').val(credit);
                            //  $(this).closest('tr').find('.display_debit').val(credit);
        ...
        $('#orders').on('click keyup','.display_credit, .display_debit',function(event){
            if (event.type === 'click' || (event.type === 'keyup' && event.key === 'Tab')) {
                if($(this).val()<=0){
                    if($(this).closest('tr').find('.display_amount').val()<=0){
                                ...
                                if(check_blance==-1){
                                    if(debit_credit_amont==0){
                                        if(parseFloat($(this).closest('tr').find('.blance').val())<credit){
                                            ...
                                            StoreCreditValue.call($(this).closest('tr'),credit)
                                            $(this).closest('tr').find('.display_credit').css({backgroundColor: 'white'});
                                        }
                                    }else{
                                        // $(this).val(credit.toFixed(amount_decimals));
                                ...
                                if(check_blance==-1){
                                    if(debit_credit_amont==0){
                                        if(parseFloat($(this).closest('tr').find('.blance').val())< debit){
                                            ...
                                            // $(this).closest('td').find('.debit').val(debit);
                                            StoreDebitValue.call($(this).closest('td'),debit)
                                    ...
                                    // $(this).val(debit.toFixed(amount_decimals));
                                    // $(this).closest('td').find('.debit').val(debit);
                                    StoreDebitValue.call($(this).closest('td'),'')
                                }
                            }
        ...
        $('#orders').on('keyup','.display_amount',function(){
            ...
            if(amount_typeabe==0){
                ...
                $(this).closest('tr').find('.rate').val(dividevalue(amount,qty));
            }
        });
        ...
        $('#orders').on('change keyup', '.display_credit, .display_debit,.qty,.rate,.display_amount', function() {
            ...
            if($(this).hasClass('display_credit') || $(this).hasClass('display_debit')){
            ...
            if($(this).hasClass('amount')){
                $(this).closest('tr').find('.rate').val(parseFloat(dividevalue(amount/(qty||0))).toFixed(amount_decimals));
            }else{
            ...
            if (parseFloat($(this).closest('tr').find('.product_id').val())) {
                ...
                $('#orders tr').each(function(i) {
                    if ($(this).find('.ledger_name').val() && $(this).closest('tr').find('.product_id').val()&& !!$(this).closest('tr').find('.amount').attr('amount')) {
                            ...
                            if (DrCr === 'Dr'){
                                // $(this).find('.display_debit').val(parseFloat(product_wise_debit_sum));
                                // $(this).find('.debit').val(parseFloat(product_wise_debit_sum));
                                StoreDebitValue.call($(this),product_wise_debit_sum)
                            }
                            else if (DrCr === 'Cr'){
                                // $(this).find('.display_credit').val(parseFloat(product_wise_debit_sum));
                            ...
                            if (prevTr) {
                                if(product_wise_debit_sum!=0){
                                    ...
                                    if (DrCr === 'Dr'){
                                        ...
                                        // prevTr.find('.display_debit').val(parseFloat(product_wise_debit_sum));
                                        StoreDebitValue.call(prevTr,product_wise_debit_sum)
                                    }
                                    else if (DrCr === 'Cr'){
                                        // prevTr.find('.credit').val(parseFloat(product_wise_debit_sum));
                                        // prevTr.find('.display_credit').val(parseFloat(product_wise_debit_sum));
...
// Cancelled journal ajax request
$(document).on('click', '.CancelIcon', function(e) {
        ...
        swal(swal_data()).then((result) => {
            if (result.value) {
                event.preventDefault();
                $.ajax({
                    url: "{{ url('voucher-journal-cancel') }}" + '/' + id ,
                    type : "POST",
