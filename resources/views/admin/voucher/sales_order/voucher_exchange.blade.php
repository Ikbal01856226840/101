...
<form id="add_sales_id" method="POST">
            ...
            <div class="col-sm-4">
                <div class="row">
                    ...
                    <div class="col-sm-8 m-0 p-0" style="margin-left: 5px;">
                        ...
                        <input type="hidden" name="sales_order_tran_id" class="form-control m-1" value="{{$data->tran_id}}" style="border-radius: 15px;"/>
                        <!-- resposive lebel change -->
    ...
    <div class="row d-none cash_ledger mt-0 pt-0" >
        <div class="col-sm-3 mt-0 pt-0"  >
            ...
            <div class="form-group" >
                ...
                disabled
               />
               <input  type="hidden"  name="salse_cash_ledger_id" class="form-control salse_cash_ledger_id" disabled/>
            ...
            <table class="table customers" style="border: none !important; margin-top: 5px;">
                <thead>
                    <tr>
                        ...
                        @else
                        <th class="th col-2 {{$voucher->remark_is==0?'d-none':'' }}">Remarks</th>
                        @endif
                    </tr>
                </thead>
        ...
        $.ajax({
            ...
            success: function(data) {
                    response($.map( data, function( item ) {
                        ...
                        object.credit_limit=item.credit_limit;
                        return object
                    }));
                }
            });
      ...
      select: function (event, ui) {
        $.ajax({
                ...
                success: function(response){
                    ...
                    $('#credit_limit').val(ui?.item?.credit_limit || 0);
                    return true;
                    }else{
...
$(document).ready(function(){
        ...
        success: function (response) {
            ...
            $.each(response.data, function(i,val) {
                $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${i}">
                    <input class="form-control  product_id m-0 p-0"  name="product_id[]" type="hidden" data-type="product_id" id="product_id_${i}" value="${val.stock_item_id||0}"  for="${i}"/>
                    <td  class="m-0 p-0"><button  type="button" name="remove" id="${i}" class="btn btn-danger btn_remove cicle m-0  py-1">-</button></td>
                    <td  class="m-0 p-0">
                        <input class="form-control product_name  autocomplete_txt" name="product_name[]" data-field-name="product_name"  type="text" data-type="product_name" value="${val.product_name||0}" id="product_name_${i}"  autocomplete="off" for="${i}"  />
                    </td>
                    <td  class="m-0 p-0 ${godown_motive==3?'d-none':''} ${godown_motive==4?'d-none':''}">
                        <input class="form-control godown_name autocomplete_txt " name="godown_name[]" value="${godown_name}"  data-field-name="godown_name" type="text"  id="godown_name_${i}"  for="${rowCount}" ${godown_motive==2?'readonly':''}  autocomplete="off"  required />
                        <input class="form-control godown_id text-right " name="godown_id[]"  data-field-name="godown_id" value="${godown_id}" type="hidden"  id="godown_id_${i}"  for="${i}" readonly />
                    </td>
                    <td  class="m-0 p-0">
                        <input
                    ...
                    <td  class="m-0 p-0">
                        <input class="form-control per " value="${val.symbol}"  name="per[]" data-field-name="per" type="text" data-type="per" id="per_${i}"  for="${i}" readonly />
                    ...
                    ${(commission_is==1)?
                    `<td  class="m-0 p-0">
                        <input class="com_credit" type="hidden" name="debit_id[]" value="${val.debit_ledger_id}">
                        <select   name="debit_ledger_id[]"  class="form-control debit_ledger_id${val.debit_ledger_id} ledger_debit" required>
                    ...
                    <td  class="m-0 p-0">
                        <div style="display: flex;flex-direction: row;">
                                <select name="product_wise_commission_cal[]" class="form-control left-data product_wise_commision_cal m-0 p-0" id="product_wise_commision_cal_${i}"  style="margin:0%;" >
                                ...
                                <input type="number" step="any" name="product_wise_commission_amount[]" id="product_wise_commission_amount_${i}" value="${val.commission||0}" class="product_wise_commission_amount form-control text-right mx-0 px-0" style="min-width:60px;" >
                        </div>
                    </td>
                    <td class="m-0 p-0"><input type="number" step="any" name="product_wise_get_commission[]"  id="product_wise_get_commission_${i}" value="${val.commission_amount||0}" class="product_wise_get_commission form-control text-right mx-0 px-0" readonly></td>
                    `:`""`}
                    <td  class="m-0 p-0 ${remark_is==0?'d-none':''}">
                        <input class="form-control remark"  name='remark[]' type="text" data-type=" id="remark_${i}"  autocomplete="off" for="${i}"/>
                    </td>
                </tr>`);
                $(`.debit_ledger_id${val.debit_ledger_id}`).val(val.debit_ledger_id);
                p=i;
                adjustFontSize($(`#godown_name_${i}`)[0],val.godown_name);
                adjustFontSize($(`#product_name_${i}`)[0],val.product_name);
                    $.ajax({
                        ...
                        async: false,
                        data: {
                            stock_item_id:val.stock_item_id,
                            godown_id:godown_id,
                        },
                        success: function(response){
                            $('#stock_'+i).val((response.data));
                        }
                    });
        ...
        for(var row=1; row<6;row++) {
            rowCount++;
                $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${rowCount}">
                    ...
                    <td  class="m-0 p-0">
                        <div style="display: flex;flex-direction: row;">
                                <select name="product_wise_commission_cal[]" class="form-control left-data product_wise_commision_cal m-0 p-0" id="product_wise_commision_cal_${rowCount}" style="margin:0%;" >
    ...
    function  commission_calcucation(){
        ...
        if(company_id==151){
            CheckCreditLimit(total_val);
        }
    }

    function CheckCreditLimit(total_val){
        let party_amount=$("#credit_amont").text().split(" ");
        let credit_amont=MakeCurrency(party_amount[0],false);
        let credit_limit=$("#credit_limit").val();
        if(party_amount[1]=="Cr"){
            if(parseFloat(credit_amont)+parseFloat(credit_limit)<total_val){
                $(":submit").attr("disabled", true);
                swal_message('Credit Limit Exceeded','error','error');
            }else{
                $(":submit").attr("disabled", false);
            }
        }else if(party_amount[1]=="Dr"){
            if(parseFloat(credit_amont)+parseFloat(total_val)>credit_limit){
                $(":submit").attr("disabled", true);
                swal_message('Credit Limit Exceeded','error','error');
            }else{
                $(":submit").attr("disabled", false);
   ...
   //append commission table
    $(document).ready(function(){
        ...
        $('#add_commission').click(function() {
            ...
            $('#commission_append').append(`<tr id="${ComRowCount}">
                    ...
                    <td colspan="${godown_motive == 3 || godown_motive == 4 ? 2 : 3}" class="m-0 p-0">
                        <input type="text" class="form-control commission_ledger_auto_complated" style="margin-bottom: 4px !important; margin-top: 2px;" required>
                        <input type="hidden" name="commission_ledger_id[]" class="form-control commission_ledger_auto_complated_id"/>
                    </td>
                    </td>
                    <td class="m-0 p-0">
                ...
                // initializeAutocomplete.call(`#commission_append tr#${ComRowCount} .commission_ledger_auto_complated`);
    ...
    function handleAutocomplete() {
        ...
        currentEle.autocomplete({
            ...
            change: function (event, ui) {
                if (ui.item == null) {
                    if($(this).attr('name')==='product_name[]'){$(this).closest('tr').find('.product_id').val('') ;$(this).closest('tr').find('.debit_ledger_id_commission').attr("required", "false");}
            ...
            select: function( event, selectedData ) {
                    if(checkDuplicateItem(selectedData?.item?.data?.stock_item_id)){
                        ...
                        if(data.ledger_head_id){
                            console.log(data.ledger_head_id);
...
$(document).ready(function(){
    $("#add_sales_id").submit(function(e) {
        ...
        $.ajax({
                url: '{{ route("voucher-exchange-store") }}',
                method: 'post',
                data: fd,
                cache: false,
                contentType: false,
