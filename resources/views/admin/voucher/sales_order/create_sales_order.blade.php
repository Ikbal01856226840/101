...
@slot('voucher_body')
<form id="add_sales_order_id" method="POST">
    @csrf
    <div class="page-body">
            ...
            <div class="col-sm-3 {{$voucher->godown_motive==3?'d-none ':'' }}">
                <div class="row" style="margin-top: 5px;">
                    ...
                    <div class="col-sm-8 m-0 p-0" style="margin-left: 5px;!important;">
                        ...
                        <select name="godown" style="margin-top: 2px; border-radius: 15px;" class="form-control m-1 js-example-basic-single js-example-basic godown" required>
    ...
    <div class="row">
        ...
        <div align="center">
             @if (user_privileges_check('Voucher',$voucher->voucher_id,'create_role'))
                <button type="submit" class="btn btn-info m-2" style="width: 116px; border-radius: 15px;">
                    <span class="m-2 m-t-1" style="color: #404040;"><i class="fa fa-save" style="font-size: 18px;"></i></span><span>Save</span>
                </button>
             @endif
            <a class="btn btn-danger" style="border-radius: 15px; m-2" href="{{route('voucher-dashboard')}}">
        ...
        for(var row=1; row<6;row++) {
            rowCount++;
                $('#orders').append(`<tr  style="margin:0px;padding:0px;" class="p-0 m-0"  id="row${rowCount}">
                    ...
                    <td  class="m-0 p-0">
                        <input class="form-control qty text-right " name="qty[]"  data-field-name="qty" type="number"
                         step="any"  id="qty_${rowCount}"  for="${rowCount}"  />
                        <input class="form-control rate text-right "  name="rate[]" data-field-name="rate"
                        type="hidden" step="any" data-type="rate" id="rate_${rowCount}"
                        for="${rowCount}" readonly/>
                    </td>
                    
                    <td  class="m-0 p-0">
                        <input class="form-control per "  name="per[]" data-field-name="per" type="text" data-type="per" id="per_${rowCount}"  for="${rowCount}" readonly />
                        <input class="form-control measure_id "  name="measure_id[]" data-field-name="measure_id" type="hidden" data-type="measure_id" id="measure_id_${rowCount}"  for="${rowCount}"  />
                    </td>
                    <td  class="m-0 p-0">
                        <input class="form-control amount  text-right" type="number" step="any"  name="amount[]"
                        id="amount_${rowCount}"  for="${rowCount}" readonly/>
                    </td>
                    <td  class="m-0 p-0">
                        ...
                        <div style="display: flex;flex-direction: row;">
                                ...
                                <input type="hidden" step="any" name="product_wise_commission_cal[]" class="form-control left-data product_wise_commision_cal m-0 p-0" id="product_wise_commision_cal_${rowCount}" style="margin:0%;" readonly>
...
function registerEvents() {
 ...
 // insert sales order
$(document).ready(function(){
    $("#add_sales_order_id").submit(function(e) {
        e.preventDefault();
        const fd = new FormData(this);
        $("#add_sales_order_btn").text('Add');
        $.ajax({
                url: '{{ route("voucher-sales-order.store") }}',
                method: 'post',
                data: fd,
                cache: false,
                contentType: false,
