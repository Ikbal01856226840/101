...
class AccountGroupSummaryController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function accountGroupSummaryWithOutOpeningBlanceIdWise(Request $request)
    {
        if (user_privileges_check('report', 'AccountGroupSummary', 'display_role')) {
            $group_chart_data = $this->groupChartRepository->getTreeSelectOption('under_id');
            $group_id = $request->group_chart_id;
            $form_date = $request->form_date;
            $to_date = $request->to_date;
            $without_opening_blance=1;

            return view('admin.report.account_summary.account_group_summary', compact('without_opening_blance','group_chart_data', 'group_id', 'form_date', 'to_date'));
        } else {
            abort(403,'Access Denied — You don’t have permission to view this page.');
        }
    }
