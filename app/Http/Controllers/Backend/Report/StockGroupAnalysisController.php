...
class StockGroupAnalysisController extends Controller
{
    ...
    public function stockGroupAnalysisIdWise(Request $request)
    {
        if (user_privileges_check('report', 'StockGroupAnalysis', 'display_role')) {
            ...
            return view('admin.report.movement_analysis_2.stock_group_analysis', compact('stock_group', 'godowns', 'stock_group_id', 'from_date', 'to_date', 'godown_id', 'purchase_in', 'grn_in', 'purchase_return_in', 'journal_in', 'stock_journal_in', 'sales_return_out', 'gtn_out', 'sales_out', 'journal_out', 'stock_journal_out'));
        } else {
            abort(403);
        }
    }
