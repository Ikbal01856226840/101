...
class StockItemAnalysisDetailsController extends Controller
{
    ...
    public function stockItemAnalysisDetailsIdWise(Request $request)
    {
        if (user_privileges_check('report', 'StockItemAnalysisDetails', 'display_role')) {
            ...
            $sort_by=$request->sort_by ?? 0;
            $sort_type=$request->stort_type ?? 0;

            return view('admin.report.movement_analysis_2.stock_item_analysis_details', compact('sort_by','sort_type','godowns', 'ledger_head_id', 'stock_item_id', 'from_date', 'to_date', 'godown_id', 'purchase_in', 'grn_in', 'purchase_return_in', 'journal_in', 'stock_journal_in', 'sales_return_out', 'gtn_out', 'sales_out', 'journal_out', 'stock_journal_out'));
        } else {
            abort(403);
        }
    }
