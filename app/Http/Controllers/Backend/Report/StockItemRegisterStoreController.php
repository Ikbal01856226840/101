...
class StockItemRegisterStoreController extends Controller
{
    ...
    public function __construct(GodownRepository $godownRepository, VoucherRepository $voucherRepository, StockItemRegisterRepository $stockItemRegisterRepository)
    {
     ...
     */
    public function StockItemRegisterStoreShow()
    {
        if (user_privileges_check('report', 'StockItemRegisterStore', 'display_role')) {
            ...
            return view('admin.report.inventrory.stock_item_register_store', compact('vouchers', 'godowns'));
        } else {
            abort(403);
        }
     ...
     * @return \Illuminate\Http\Response
     */
    public function StockItemRegisterStore(Request $request)
    {
        if (user_privileges_check('report', 'StockItemRegisterStore', 'display_role')) {
            try {
