...
class StockJournalController extends Controller
{
    ...
    {
        if ((!empty(array_filter($request->product_in_id)))||(!empty(array_filter($request->product_out_id)))) {
    ...
    public function edit($id)
    {
        ...
        if (user_privileges_check('Voucher', $data->voucher_id, 'display_role')) {
            ...
            if ($voucher->is_pary_ledger == 1) {
                $ledger = $this->voucherStockJournalRepository->getLedgerTranIdWise($id);
            } else {
                $ledger = 0;
            }

            return view('admin.voucher.stock_journal.edit_stock_journal', compact('branch_setup', 'data', 'voucher', 'godowns', 'customers', 'distributionCenter', 'voucher', 'godown_id_in', 'destination_godowns', 'ledger'));
        } else {
            abort(403);
        }
    }
    ...
    public function destinationPrice(Request $request)
    {
        ...
        $get_price = $this->voucher_setup->stockItemPrice($request->stock_item_id, $price_setup->destrination_price_type_id,0,0,$request->tran_date??'');
        echo json_encode($get_price);
        exit();
    }

    public function Cancel(Request $request){
        $data = $this->voucherStockJournalRepository->getStockJournalId($request->id);
        if (user_privileges_check('Voucher', $data->voucher_id, 'delete_role')) {
            try {
    ...
    public function billOfMaterialSearching(Request $request){
        try {

            $data=$this->voucher_setup->bill_of_material_searching($request->name);
            return RespondWithSuccess('bill of material searching successful  !! ', $data, 201);
        } catch (Exception $e) {
            return RespondWithError('bill of material searching successful !!', $e->getMessage(), 404);
        }
    }

    public function billOfMaterialQty(Request $request){
        try {

            $data=$this->voucher_setup->bill_of_material_qty($request->id,$request->voucher_id,$request->godown_id,$request->godown_out);
            return RespondWithSuccess('bill of material searching successful  !! ', $data, 201);
        } catch (Exception $e) {
            return RespondWithError('bill of material searching successful !!', $e->getMessage(), 404);
        }
