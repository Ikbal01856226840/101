...
class SalesOrderController extends Controller
{
    ...
    public function show($id)
    {
        if (user_privileges_check('Voucher', $id, 'display_role')) {
            ...
            return view('admin.voucher.sales_order.create_sales_order', compact( 'voucher_invoice','voucher_date', 'branch_setup', 'voucher','debit_sum_value','godowns'));
        } else {
            abort(403);
        }
    }
    ...
    public function edit($id)
    {
        ...
        if (user_privileges_check('Voucher', $data->voucher_id, 'display_role')) {
            ...
            return view('admin.voucher.sales_order.edit_sales_order', compact('branch_setup', 'data', 'voucher', 'credit_sum_value','godowns'));
        } else {
            abort(403);
        }
    }
    ...
    public function  VoucherExchance(Request $request){
        ...
        // if (user_privileges_check('Voucher', $data->voucher_id, 'alter_role')) {
            ...
            $balanceDebitCredit =$this->voucher_setup->balanceDebitCredit($data->ledger_head_id??0) ;
            $debit_sum_value = $balanceDebitCredit ? $this->voucher_setup->balanceDebitCreditCalculation($balanceDebitCredit) : '';
        ...
        if (count(array_filter($request->product_id))!= 0) {
            if (user_privileges_check('Voucher', $request->voucher_id, 'create_role')) {
                    ...
                    $validator = Validator::make($request->all(), [
                        ...
                        'credit_ledger_id' =>  ['required', 'numeric', 'not_in:0'],
