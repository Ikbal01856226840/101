...
class PaymentController extends Controller
{
    ...
    public function __construct(GodownRepository $godownRepository, BranchRepository $branchRepository, Voucher_setup $voucher_setup, VoucherPaymentRepository $voucherPaymentRepository)
    {
    ...
    public function show($id)
    {
        if (user_privileges_check('Voucher', $id, 'display_role')) {
            ...
            if($voucher->voucher_with_stock_is==1){
                ...
                return view('admin.voucher.payment.create_payment_with_stock', compact('godowns', 'voucher_date', 'branch_setup', 'voucher_invoice', 'voucher'));
            }else{
    ...
    public function edit($id)
    {
        ...
        if (user_privileges_check('Voucher', $data->voucher_id, 'display_role')) {
            ...
            if($voucher->voucher_with_stock_is==1){
                $godowns = $this->voucher_setup->godownAccess($voucher->voucher_id);
                return view('admin.voucher.payment.edit_payment_with_stock', compact('godowns', 'data', 'branch_setup', 'voucher'));
            }else{
    ...
    {
        if (empty(array_filter($request->ledger_id))) {
            return RespondWithError('Ledger not Empty', 'Ledger not Empty', 404);
        }
        if (!validateTransaction($request)) {
