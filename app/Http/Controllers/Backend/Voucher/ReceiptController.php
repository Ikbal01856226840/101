...
class ReceiptController extends Controller
{
    ...
    public function __construct(GodownRepository $godownRepository, BranchRepository $branchRepository, Voucher_setup $voucher_setup, AuthRepository $authRepository, VoucherReceivedRepository $voucherReceivedRepository)
    {
    ...
    public function store(Request $request)
    {
        if (empty(array_filter($request->ledger_id))) {
            return RespondWithError('Ledger is Empty', 'Ledger is Empty', 404);
        }

        if ((count(array_filter($request->credit)) + count(array_filter($request->debit))) != count(array_filter($request->ledger_id))) {
            return RespondWithError('Ledger is Empty', 'Ledger is Empty', 404);
        }
        if (!validateTransaction($request)) {
            return RespondWithError('Debit Credit Not Equal', 'Credit Not Equal', 404);
        }
    ...
    public function show($id)
    {
        if (user_privileges_check('Voucher', $id, 'display_role')) {
            ...
            if($voucher->voucher_with_stock_is==1){
                $godowns = $this->voucher_setup->godownAccess($voucher->voucher_id);
                return view('admin.voucher.receipt.create_receipt_with_stock', compact('godowns', 'voucher_date', 'branch_setup', 'voucher_invoice', 'voucher'));
            }else{
    ...
    public function edit($id)
    {
        ...
        if (user_privileges_check('Voucher', $data->voucher_id, 'display_role')) {
            ...
            if($voucher->voucher_with_stock_is==1){
                $godowns = $this->voucher_setup->godownAccess($voucher->voucher_id);
                return view('admin.voucher.receipt.edit_receipt_with_stock', compact('godowns', 'data', 'branch_setup', 'voucher'));
            }else{
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!validateTransaction($request)) {            
            return RespondWithError('Debit and Credit Not Equal', 'Debit and Credit Not Equal', 404);
        }
        
        if (((count(array_filter($request->credit, 'filterNullAndZero')) + count(array_filter($request->debit, 'filterNullAndZero'))) != count(array_filter($request->ledger_id)))) {
            return RespondWithError('Ledger is Empty', 'Ledger is Empty', 404);
        }
    ...
    public function searchingLedgerDataGet()
    {
        ...
        if ($_GET['fieldName'] == 'product_name') {
            ...
            if (array_sum(array_map('intval',explode(' ', $debit_group->debit_group_id))) != 0 || array_sum(array_map('intval',explode(' ', $debit_group->credit_group_id))) != 0) {
                if ($_GET['DrCr'] == 'Dr') {
    ...
    public function Cancel(Request $request){
        $data = $this->voucherReceivedRepository->getVoucherReceivedId($request->id);
        if (user_privileges_check('Voucher', $data->voucher_id, 'delete_role')) {
            try {
