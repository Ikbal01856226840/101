...
class JournalController extends Controller
{
    ...
    public function __construct( Voucher_setup $voucher_setup, Tree $tree, GroupChartRepository $groupChartRepository, CustomerRepository $customerRepository, UserCheck $userCheck, VoucherJournalRepository $voucherJournalRepository, VoucherReceivedRepository $voucherReceivedRepository)
    {
    ...
    {
        if (!empty(array_filter($request->ledger_id)) && (array_sum($request->credit)==array_sum($request->debit))&& ((count(array_filter($request->credit))+count(array_filter($request->debit)))==count(array_filter($request->ledger_id)))) {
            ...
            return RespondWithError('Ledger not Empty OR Debit and Credit Not Equal !!', 'Ledger not Empty OR Debit and Credit Not Equal', 404);
        }
    }
    ...
    public function edit($id)
    {
        ...
        if (user_privileges_check('Voucher', $data->voucher_id, 'display_role')) {
            ...
            $balanceDebitCredit = $this->voucher_setup->balanceDebitCredit($debit_credit_data[1]->ledger_head_id??0);
    ...
    {    
        if (!empty(array_filter($request->ledger_id)) && (array_sum($request->credit)==array_sum($request->debit))&& ((count(array_filter($request->credit,'filterNullAndZero'))+count(array_filter($request->debit,'filterNullAndZero')))==count(array_filter($request->ledger_id)))) {
            $voucher_invoice = '';
            if ($request->ch_4_dup_vou_no == 0) {
    ...
    public function getJournalData(Request $request)
    {
        ...
        if ($request->stock_item_id) {
            $stock = $this->voucher_setup->stock_in_stock_out_sum_qty($request->stock_item_id,$request->godown_id);
        } else {
            $stock = 0;
        }
    ...
    {
        $data = $this->voucherJournalRepository->getJournalId($request->id);
        if (user_privileges_check('Voucher', $data->voucher_id, 'delete_role')) {
            try {
                $data = $this->voucher_setup->cancelStockinAndStockOutDebitCredit($request->id);
