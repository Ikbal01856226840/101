<?php

namespace App\Rules;

use Closure;
use Illuminate\Contracts\Validation\ValidationRule;

class LedgerSalesValidation implements ValidationRule
{
    protected $data; // Holds additional request data
    protected $errors = []; // Stores validation errors for each row
    /**
     * Constructor to accept additional request data.
     *
     * @param array $data
     */
    public function __construct(array $data)
    {
        $this->data = $data;
    }

    /**
     * Run the validation rule.
     *
     * @param  string  $attribute
     * @param  mixed  $value
     * @param  \Closure(string): \Illuminate\Translation\PotentiallyTranslatedString  $fail
     */
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {


        $productNames = $this->data['product_name'] ?? [];

        $total = $this->data['amount'] ?? [];

        $get_commission=$this->data['get_commission']??[];
        $commision_cal=$this->data['commision_cal']??[];
        $commission_ledger_id=$this->data['commission_ledger_id']??[];
        $commission_is=$this->data['commission_is']??[];
        $debit_ledger_id=$this->data['debit_ledger_id']??[];
        $credit_ledger_id=$this->data['credit_ledger_id']??[];

        $product_wise_get_commission=$this->data['product_wise_get_commission']??[];
        $product_wise_commission_ledger=$this->data['product_wise_commission_ledger']??[];
        $product_wise_commission_cal=$this->data['product_wise_commission_cal']??[];
        $total_amount=$this->data['total_amount']??0;
        $get_without_commission=$this->data['get_without_commission']??0;


        $total_debit = 0;
        $total_credit = 0;
        if (!empty($get_commission)) {
            foreach ($get_commission as $i => $commissionAmount) {
                if (!empty($commissionAmount)) {
                        $commission_ledger_ids = $commission_ledger_id[$i] ?? null;
                        $isDebit = in_array($commision_cal[$i], [1, 3]);
                        $total_debit+= $isDebit ? (double)$commissionAmount : 0;
                        $total_credit += $isDebit ? 0 : (double)$commissionAmount;
                        empty($commission_ledger_ids) ? $this->errors[] = "The commission ledger empty  at Row Number " . ($i + 1) . " is required.<br>" : '';
                }
            }
        }

        if ($commission_is != 1) {
            $total_credit+= (float) $total_amount ?? 0;
            if (empty($debit_ledger_id)) {
                 $this->errors[] = "The Sales Ledger is empty <br>";
            }
        }

        if (empty($credit_ledger_id)) {
           $this->errors[] = "The Party's A/C Name is empty <br>";
        }

        $total_debit+= (float) $get_without_commission ?? 0;


        if ($commission_is == 1) {
            foreach ($productNames as $index => $productName) {

                $row = $index + 1;
                $totals = $total[$index] ?? null;
                $debit_ledger_ids=$debit_ledger_id[$index] ?? null;
                $product_wise_commission_ledgers=$product_wise_commission_ledger[$index] ?? null;
                $product_wise_get_commissions=$product_wise_get_commission[$index] ?? null;

                if (!empty($productName)) {
                    if ($commission_is == 1) {
                        $total_credit +=(double)$totals??0;
                        if (empty($debit_ledger_ids)) {
                            $this->errors[] = $this->errors[] = "The sales ledger empty  at Row Number " .$row. " is required.<br>";
                        }
                        $isDebit = in_array($product_wise_commission_cal[$index], [1, 3]);
                        $total_debit+= $isDebit ?(double) $product_wise_get_commissions : 0;
                        $total_credit += $isDebit ? 0 :(double) $product_wise_get_commissions;
                        empty($product_wise_commission_ledgers) ? $this->errors[] = "The commission ledger empty  at Row Number " . $row . " is required.<br>" : '';
                    }
                }
            }
        }

        if (abs(((double)$total_debit)-((double)$total_credit))> 1) {
            $this->errors[] = "Total debit credit calculation does not equal.<br>";
          }
        // Pass errors to the $fail closure
        foreach ($this->errors as $error) {
            $fail($error);
        }
    }


}
