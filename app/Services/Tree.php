...
class Tree
{
    ...
    public function getTreeViewSelectOptionGroup_chart_two($data, $parentId,$under_id = null, $level = 0, $t = 0)
    {
        $options = '';
        foreach ($data as $item) {
            ...
            if ($item['under'] == $parentId) {
                if ($item['group_chart_id'] != $t) {
                    $prefix = str_repeat('&nbsp;&nbsp;&nbsp', $level); // add a prefix to indicate level
                    if (! empty($under_id)) {
                      $options .= '<option value="'.$item['group_chart_id'].'-'.$item['under'].'">'.$prefix.$item['group_chart_name'].'</option>';
                    }else{
                      $options .= '<option value="'.$item['group_chart_id'].'">'.$prefix.$item['group_chart_name'].'</option>';
                    }
                    $options .= $this->getTreeViewSelectOptionGroup_chart_two($data, $item['group_chart_id'],$under_id, $level + 1, $t); // recursively add child options
                    $t = $item['group_chart_id'];
                }
            }
        }
    ...
    public function getTreeViewSelectOptionRetrailerTree($data, $parentId, $level = 0, $t = 0)
    {
        $options = '';
        foreach ($data as $item) {
            ...
            if ($item['under'] == $parentId) {
                if ($item['group_chart_id'] != $t) {
                    ...
                    SELECT *
                    FROM   ledger_head
                    WHERE  ledger_head.group_id = ?
                           AND ledger_head.ledger_type <> 0
                           AND (ledger_head.ledger_type = 2
                                OR EXISTS (
                                    SELECT l.under_ledger_id
                                    FROM   ledger_head AS l
                                    WHERE  l.ledger_type = 2
                                           AND ledger_head.ledger_head_id = l.under_ledger_id
                                )
                           )", [$item['group_chart_id']]);;
                    ...
                    $options .= $this->getTreeViewSelectOptionRetrailerTree($data, $item['group_chart_id'], $level + 1, $t); // recursively add child options
                    $t = $item['group_chart_id'];
                }
            }
     ...
     public function stock_group_tree_row_query($group)
    {
        $data = DB::select("
                            WITH RECURSIVE tree AS (
                                ...
                                FROM 
                                    stock_group gc
                                WHERE 
                                    FIND_IN_SET(gc.stock_group_id, :group)
                                
                                UNION ALL
                                
                                SELECT 
                                    child.stock_group_id,
                                    child.stock_group_name,
                                    child.under,
                                    parent.lvl + 1 AS lvl
                                FROM 
                                    tree parent
                                JOIN 
                                   stock_group child ON parent.stock_group_id = child.under
                            )
                            SELECT * FROM tree
                        ", ['group' => $group]);
    ...
    public function getTreeViewSelectOptionStock_group_two($data, $parentId,$under_id = null, $level = 0, $t = 0)
    {
        $options = '';
        foreach ($data as $item) {

            if ($item['under'] == $parentId) {
                if ($item['stock_group_id'] != $t) {
                    $prefix = str_repeat('&nbsp;&nbsp;', $level);
                    if (!empty($under_id)) {
                      $options .= '<option value="'.$item['stock_group_id'].'-'.$item['under'].'">'.$prefix.$item['stock_group_name'].'</option>';
                    }else{
                      $options .= '<option value="'.$item['stock_group_id'].'">'.$prefix.$item['stock_group_name'].'</option>';
                    }
                    $options .= $this->getTreeViewSelectOptionStock_group_two($data, $item['stock_group_id'],$under_id, $level + 1, $t); 
                    $t = $item['stock_group_id'];
                }
            }
        }
        return $options;
