...
{
    public function OpeningRate($from_date, $to_date)
    {
        $query= "WITH s_summary AS (
            ...
            FROM stock
            WHERE tran_date < :op_from_date_2
        ...
        s_latest AS (
            SELECT * FROM (
                ...
                FROM stock
                WHERE tran_date < :op_from_date_3
            ) ranked
            WHERE rn = 1
        ),
        weighted_rates AS (
       ...
       SELECT SUM(closing_balance.current_rate*closing_balance.stock_qty_opening) AS  total_stock_total_out_opening FROM(SELECT
                   cte.current_rate,
                   cte.stock_qty_opening
        FROM stock_group sg
