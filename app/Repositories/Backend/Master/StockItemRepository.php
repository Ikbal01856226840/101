...
use App\Models\MirrorStockItem;
...
class StockItemRepository implements StockItemInterface
{
    ...
    public function updateStockItem(Request $request, $id)
    {
        ...
        $MirrorStockItem=new MirrorStockItem();
        $MirrorStockItem->type='update';
        $MirrorStockItem->item_id=$id;
        $MirrorStockItem->user_name=Auth::user()->user_name;
        $MirrorStockItem->user_id=Auth::id();
        $MirrorStockItem->other_details=json_encode('<br> Updated On: '.\Carbon\Carbon::now()->format('D, d M Y g:i:s A').', By: '.Auth::user()->user_name.', Ip: '.$ip);
        $MirrorStockItem->old_data=json_encode($mirrorData);
        $MirrorStockItem->new_data=json_encode($data);
        $MirrorStockItem->save();
        return $data;
    }

    public function deleteStockItem($id)
    {
       $stock= DB::table('stock')->where('stock_item_id', $id)->first();
       if(empty($stock)){
            $ip = $_SERVER['REMOTE_ADDR'];
            $mirrorData=StockItem::findOrFail($id);
            $MirrorStockItem=new MirrorStockItem();
            $MirrorStockItem->type='delete';
            $MirrorStockItem->item_id=$id;
            $MirrorStockItem->user_name=Auth::user()->user_name;
            $MirrorStockItem->user_id=Auth::id();
            $MirrorStockItem->other_details=json_encode('<br> Updated On: '.\Carbon\Carbon::now()->format('D, d M Y g:i:s A').', By: '.Auth::user()->user_name.', Ip: '.$ip);
    ...
    public function getStockItemData()
    {
        return DB::select('SELECT stock_item.bangla_product_name,stock_item.other_details,stock_item.user_name,stock_item.alias,
                                        ...
                                        LEFT JOIN unitsof_measure ON stock_item.unit_of_measure_id=unitsof_measure.unit_of_measure_id ORDER BY stock_group.stock_group_name DESC,stock_item.product_name DESC');
    }

    public function getSpecificItemData()
    {
        $stock_group = DB::table('stock_group')->select('stock_group.stock_group_id', 'stock_group.stock_group_name', 'stock_group.under', 'stock_item.stock_item_id', 'stock_item.product_name')->leftJoin('stock_item', 'stock_group.stock_group_id', '=', 'stock_item.stock_group_id')->orderBy('stock_group.stock_group_name', 'DESC')->orderBy('stock_item.product_name', 'DESC')->get();
        $stock_group_object_to_array = json_decode(json_encode($stock_group, true), true);
