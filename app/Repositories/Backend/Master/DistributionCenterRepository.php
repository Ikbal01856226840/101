...
use App\Models\MirrorDistributionCenter;
...
class DistributionCenterRepository implements DistributionCenterInterface
{
    ...
    public function updateDistributionCenter(Request $request, $id)
    {
        ...
        $MirrorDistributionCenter=new MirrorDistributionCenter();
        $MirrorDistributionCenter->type='update';
        $MirrorDistributionCenter->distribution_center_id=$id;
        $MirrorDistributionCenter->user_name=Auth::user()->user_name;
        $MirrorDistributionCenter->user_id=Auth::id();
        $MirrorDistributionCenter->other_details=json_encode('<br> Updated On: '.\Carbon\Carbon::now()->format('D, d M Y g:i:s A').', By: '.Auth::user()->user_name.', Ip: '.$ip);
        $MirrorDistributionCenter->old_data=json_encode($mirrorData);
        $MirrorDistributionCenter->new_data=json_encode($data);
        $MirrorDistributionCenter->save();
    ...
    {
        $ip = $_SERVER['REMOTE_ADDR'];
        $mirrorData=DistributionCenter::findOrFail($id);
        $MirrorDistributionCenter=new MirrorDistributionCenter();
        $MirrorDistributionCenter->type='delete';
        $MirrorDistributionCenter->distribution_center_id=$id;
        $MirrorDistributionCenter->user_name=Auth::user()->user_name;
        $MirrorDistributionCenter->user_id=Auth::id();
        $MirrorDistributionCenter->other_details=json_encode('<br> Updated On: '.\Carbon\Carbon::now()->format('D, d M Y g:i:s A').', By: '.Auth::user()->user_name.', Ip: '.$ip);
