        ...
        $data = DB::select("WITH recursive tree
                            ...
                            LEFT JOIN
                                        (
                                                ...
                                                WHERE     voucher_setup.voucher_type_id IN (1,6,8,14) 
                            ...
                            ORDER BY  group_chart_name DESC",['from_date'=>$from_date,'to_date'=> $to_date,'group_id'=>$group_id]);
        
        $group_chart_object_to_array = json_decode(json_encode($data, true), true);
        $data = $this->tree->buildTree($group_chart_object_to_array, $group_id, 0, 'group_chart_id', 'under', 'ledger_head_id');
