...
class PartyLedgerRepository implements PartyLedgerInterface
{
    public function PartyLedgerOfIndex($request = null)
    {
        ...
        if ($request->ledger_id == 0) {
            // return  DB::select('CALL GetLedgerReport(?, ?)', array($from_date,$to_date));
            ...
            $party_ledger = DB::select(
                "SELECT transaction_master.tran_id,
                                                ...
                                                CASE
                                                    WHEN debit_credit.dr_cr = 'Dr' THEN
                                                        ...
                                                        AND debit_credit.tran_id = transaction_master.tran_id
                                                        GROUP BY transaction_master.tran_id)
                                                    ELSE
                                                        (SELECT ledger_head.ledger_name
                                                        FROM debit_credit
                                                        LEFT JOIN ledger_head ON debit_credit.ledger_head_id = ledger_head.ledger_head_id
                                                        WHERE debit_credit.dr_cr = 'Dr'
                                                        AND debit_credit.tran_id = transaction_master.tran_id
                                                        GROUP BY transaction_master.tran_id)
                                                END AS ledger_name,
                                                ledger_head.DrCr,
                                        ...
                                        GROUP  BY transaction_master.tran_id",['from_date'=>$from_date,'to_date'=>$to_date,'ledger_id'=>$request->ledger_id]
